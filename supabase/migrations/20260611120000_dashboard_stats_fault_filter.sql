-- Add fault_party filter to dashboard stats RPC (matches list/export filters).

drop function if exists public.incident_dashboard_stats(date, date, text, text, text, uuid);
...
grant execute on function public.incident_dashboard_stats(date, date, text, text, text, text, uuid) to authenticated;
