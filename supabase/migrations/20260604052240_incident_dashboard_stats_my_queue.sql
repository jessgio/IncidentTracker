-- Adds p_user_id parameter and my_queue count to the dashboard stats RPC.
-- Replaces the earlier 5-param version so the client can show "My Queue" KPIs.

drop function if exists public.incident_dashboard_stats(date, date, text, text, text);
