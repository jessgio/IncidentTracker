-- Add per-PIC average first response and SLA breach counts to dashboard stats.
...
as $$
  ...
  pic_first_response as (
    select
      fr.pic_id,
      coalesce(nullif(trim(pr.full_name), ''), pr.email, 'Unassigned') as pic_name,
      round(avg(fr.hours)::numeric, 1) as avg_hours
    from first_response fr
    left join public.profiles pr on pr.id = fr.pic_id
    where fr.hours is not null
    group by fr.pic_id, pr.full_name, pr.email
  ),
  pic_sla as (
    select
      ob.assigned_to as pic_id,
      coalesce(nullif(trim(pr.full_name), ''), pr.email, 'Unassigned') as pic_name,
      count(*)::int as sla_breaches
    from open_base ob
    left join public.profiles pr on pr.id = ob.assigned_to
    where coalesce(ob.status_changed_at, ob.created_at) < now() - interval '3 days'
    group by ob.assigned_to, pr.full_name, pr.email
  )
      ...
      from (
        select
          jsonb_build_object(
            ...
            'total_active', sum(c),
            'avg_first_response_hours', (
              select pfr.avg_hours
              from pic_first_response pfr
              where pfr.pic_id is not distinct from ps.pic_id
                and pfr.pic_name = ps.pic_name
            ),
            'sla_breaches', coalesce((
              select psa.sla_breaches
              from pic_sla psa
              where psa.pic_id is not distinct from ps.pic_id
                and psa.pic_name = ps.pic_name
            ), 0),
            'by_status', (
              select coalesce(
